...
async function requireUser() {
  ...
  if (user.app_metadata?.role === "admin") { window.location.href = "admin.html"; return null; }
  return user;
}
...
function render(items) {
  ...
  for (const it of items) {
    ...
    row.innerHTML = `
      ...
      <div class="right">
        <button class="btn" data-op="minus" data-item="${it.item_no}">–</button>
        <div class="qty" id="qty-${it.item_no}">${it.qty}</div>
        <button class="btn" data-op="plus" data-item="${it.item_no}">+</button>
      </div>
    `;
...
async function updateQty(userId, itemNo, delta) {
...
(async () => {
  ...
  $("list").addEventListener("click", async (ev) => {
    ...
    try {
      await updateQty(user.id, itemNo, delta);
    } catch (e) {
